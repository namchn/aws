...
import com.example.demo.project.todo.service.TodoService;
...
public class TodoController {
  ...
  public ResponseEntity<?> createTodo(@AuthenticationPrincipal String userId,@RequestBody TodoDTO dto) {
    try {
      //String temporaryUserId = "temporary-user"; // temporary user id.
    	...
    	//Log.info(userId+"made todo"  );	
    	
      // (1) TodoEntity로 변환한다.
      TodoEntity entity = TodoDTO.toEntity(dto);
      
      String errorMassege = "";
      // 널값 처리  // 이건 다른 로직 수정이 필요함 .
      if(entity.getTitle() == null ||  entity.getTitle().equals("") ) {
    	  errorMassege = "empty_title";
          ResponseDTO response = ResponseDTO.builder().error(errorMassege) //""
        		  									.build();
          return ResponseEntity.ok().body(response);
      }
      ...
      //제네릭으로 만들기.
      //List<TodoEntity> aa = service.ccc2(entity);
  ...
  public ResponseEntity<?> retrieveTodoList(@AuthenticationPrincipal String userId) {
	...
	// (1) 서비스 메서드의  retrieve 매서드를 사용해 Todo리스트를 가져온다.
